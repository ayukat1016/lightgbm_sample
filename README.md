...
| 5  | p112 SHAP expected_value | expected_valueでテストデータ全件レコードに対する予測値の期待値を表示します | expected_value は、説明対象データそのものから計算される平均値ではなく、SHAPが基準とするモデル出力の期待値です。tree_path_dependent の場合、この値は主に学習済みモデルの構造に依存して決まります。なお、実際の計算は explainer(X) の実行時に行われるため、入力時に決まるように見える点に注意が必要です。 | 不正確な記載なので修正 |
| 6  | p129 前処理後のincome件数内訳の実行結果 | <=50K   24720、>50K      7841 | <=50K   21999、>50K      7171 | 誤って前処理前の件数を記載  |
| 7  | p306 中央 | インデックスj関係は以下の関係になります。 | インデックスjは以下の関係になります。 | 誤字  |
| 8  | p340 1本目の木の可視化 | figsize=(20,20) | figsize=(20, 20) | スペースを追加  |
| 9  | p303 XGBoostの改善点 | ①目的関数が最小化する重みでレコードを2分割するので、回帰木が不要になった | ①XGBoostも回帰木を使用する。損失関数の1次微分・2次微分を用いて、回帰木の分割点と葉の重みを決定する | 「回帰木が不要になった」という記載が不正確。XGBoostも決定木を弱学習器として使用する |
| 10  | p303 XGBoostの改善点 | XGBoostはWeighted Quantile Sketch、LightGBMはヒストグラムを使って特徴量の数値を離散化| XGBoostはWeighted Quantile Sketchなどを用いた近似分割探索に加え、ヒストグラムを用いた分割探索も利用できます。LightGBMはヒストグラムベースの学習を中心に高速化しています | XGBoostにも tree_method=hist があり、特徴量をbin化して分割点を探索します  |
| 11 | p313 損失関数の微分 | 絶対誤差の2次微分：1 | 絶対誤差の2次微分：誤差が0でない点では0、誤差0では微分不可 | XGBoostでは絶対誤差を滑らかに近似して最適化します。LightGBMでは学習用のhessianとして1を使用します。 |
| 12 | p330、p354 ヒストグラムによる学習の高速化 | 1回のブースティングに必要な計算量が $`O(n\times m)`$ から $`O(bins\times m)`$ に削減できる | ヒストグラムを用いない場合、分割点探索の計算量は $`O(n\times m)`$ ですが、ヒストグラム構築後は $`O(bins\times m)`$ に削減できます。 | ヒストグラム構築自体には $`O(n\times m)`$ の計算が必要なため、1回のブースティング全体の計算量が $`O(bins\times m)`$ になるわけではありません |
| 13 | p331 最下部 | 縦軸のレコード件数は分割点の計算に使用しません。 | 二乗誤差の場合、bin内のレコード件数は2次微分（hessian）の合計値と一致するため、分割点の計算に関係します。 | LightGBMは各binの1次微分（gradient）と2次微分（hessian）の合計値を用いて分割点を評価します。 |
| 14 | p331、p335-336 ヒストグラムによる学習の高速化 | ヒストグラムが無効化します | max_binを大きくし、min_data_in_bin=1とすることでbinを細かくでき、この例ではヒストグラムを使わない場合と同じ分割点や予測値を再現できます。 | LightGBMは引き続きヒストグラムを用いて分割点を探索しており、ヒストグラム自体が無効化されるわけではありません。 |
...
| 2026-04-29 　 | SHAP expected_valueの修正を正誤表に追加        |
| 2026-09-13 　 | 5章の不正確な記載（No9~14）を正誤表で訂正        |